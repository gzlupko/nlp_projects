...
# Empirical Paper 
## Using Natural Language Processing to Identify the Substantive Reasoning Underlying Behavior: An Application to Employees’ Behavior to Work-From-Home During COVID-19
...
# update: dec 2022: retain NA in text data to overcome potential range restriction
...
# also export a data set with the participants that did not provide reasons
# but did provide responses to the other study variables (this is to mitigate range restriction)

no_reason_provided <- reasons_formatted %>%
  subset(is.na(Reason_Stated)) 
#write.xlsx(no_reason_provided, "no_reason_provided.xlsx")
...
The next set of pre-processing steps will remove rows without a stated reason. However, due to potential range restriction in the relationship between stated reasons and T2 behavior, 

```{r}
no_reason_for_provided <- reasons_with_features %>%
  subset(is.na(document))
...
min_thresh_plot_reasons_for <- plotRemoved(processed$documents, 
                                           lower.thresh = seq(1, 10, by = 1)) 

thresh_dat <- data.frame(min_thresh_plot_reasons_for$nwords) 
thresh_dat <- thresh_dat %>% 
  rename(words_removed = min_thresh_plot_reasons_for.nwords) %>%
  mutate(min_docs = 1:10)
thresh_dat

# plot the words removed as function of min docs words appear 
thresh_dat %>% 
  ggplot(aes(x = min_docs, y = words_removed)) + 
  geom_line() + theme_apa() + 
  scale_x_continuous(breaks = seq(min(thresh_dat$min_docs), max(thresh_dat$min_docs), by = 1)) + labs(x = "Minimum No. Documents", y = "Number of Words Removed") 


thresh_dat %>% 
  ggplot(aes(x = min_docs, y = words_removed)) + 
  geom_line() + 
  scale_x_continuous(breaks = seq(min(thresh_dat$min_docs), max(thresh_dat$min_docs), by = 1)) +
  scale_y_continuous(breaks = seq(min(0), max(thresh_dat$words_removed), by = 50)) +
  theme_linedraw() + labs(x = "Minimum No. Documents", y = "Number of Words Removed") + 
  theme_apa() 

# set lower threshold of 3, requiring words to appear in at least 3 reasons stated in order to be included in the analysis
...
STM Insights 
```{r}
# export an .Rdata file with the stm model objects included 
save.image(file = "empirical_paper.Rdata")
library(stminsights)
run_stminsights()
```
...
# descriptives reasons for data 
reasons_for_data %>% 
  ggplot(aes(x = behavior_ave)) + geom_histogram(stat = "bin", binwidth = 1)   

describe(reasons_for_data$behavior_ave) 
...
Update, December 2022: those that did not provide reasons but that had T2 behavior were originally removed from the data set for follow-up correlations and regressions. This may have seriously range restricted the relationships between the topics and the study variables. 

so far data with missing reasons was attached manually in excel to data that has been processed and assigned the topic proportions. 
...
# format data with missing columns so that it attaches with cbind() to the processed reasons data 
# extract study variables to manually combine two data sets
#write.xlsx(no_reason_for_provided, "no_reason_provided.xlsx")



reasons_full_with_missing <- read_excel("reasons_full_with_missing.xlsx")

head(reasons_full_with_missing)
...
lm(behavior_ave ~ job_sat + burnout, data = reasons_full_with_missing) %>%
  summary()

# compare to data with no missing values retained 

lm(behavior_ave ~ job_sat + burnout, data = reasons_for_data) %>%
  summary()
...
# where hold out group is
lm(behavior_ave ~ Topic1 + Topic2, data = reasons_for_data) %>% summary()
lm(behavior_ave ~ Topic1 + Topic2, data = reasons_full_with_missing) %>% summary() 
...
# hierarchical MR 
# mod1 = job sat and burnout 
mod1 <- lm(behavior_ave ~ burnout + job_sat, data = reasons_for_data)
mod1 %>% summary() 

# mod2 = text features added 

mod2_reasons_for <- lm(behavior_ave ~ sent_vader + n_words + n_first_person + n_first_personp + n_second_personp + n_third_person, data = reasons_for_data)
mod2_reasons_for %>% summary() 
...
mod2_reasons_against <- lm(behavior_ave ~ sent_vader + n_words + n_uq_words + n_first_person + n_first_personp + n_second_personp + n_third_person, data = reasons_against_full)
summary(mod2_reasons_against) 
...
reasons_against_full$n_uq_words
```
