...
topics_assigned$topic_factor <- factor(x = topics_assigned$topic_assigned, 
                          ...
                          labels = c("Topic1", "Topic2", "Topic3", "Topic4", "Topic5", "Topic6", "Topic7", "Topic8")) 
...
```{r Standardized-Beta-Values}

stdcoeff <- function(dataset = dat, 
                     predictor = x, 
                     outcome =y) { 
  
  dat$standard_y <- scale(dat$y)
  dat$standard_x <- scale(dat$x)
...
class(topics_assigned$like_count) 
```
