...
# check remove duplicate tweets 

# remove twitter specific stop words 
twitter_cleaned <- twitter_data %>%
  mutate(cleaned = 
           str_remove_all(text, "t.co | https | i'm | amp | it's | don't | can't | 
                          home from work | IL | II | lol | didn't | ain't | 
                          ve | doesn't | at | i"))    
...
twitter_2 <- twitter_cleaned %>%
  mutate(text1 = str_remove_all(text, " home | amp | II | don | dai"))  
